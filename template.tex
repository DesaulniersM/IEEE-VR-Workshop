...
\abstract{
  ...
  between users. This paper describes efforts to lower human perceived end-to-end latency of a multi-user AR system. %QH{This paper describes efforts to design and fully implement a benchmark multi-user AR system. this is not an accurate claim of this work's contribution since this sounds like the entire work is about implementing the benchmark system} 
  The system is measured with respect to user perceived latency as a motivation for directing efforts for future system-level research 
  as well as improvements. We find that the latency bottleneck is associated with the cloud server based user alignment 
  system.
   % \QH{and augment this system with a peer-to-peer user alignment scheme. should break this up here.  the flow should be that we identified the bottleneck, then what we did to address that.} 
  ...
  discuss future work on how the system may be made more robust to scaling and network conditions. %\QH{in technical writing, we use active voice, not passive voice. so instead of ``future work is discussed", we say ``we discuss future work"}
  latency
} % end of abstract

%% Keywords that describe your work. Will show as 'Index Terms' in journal
%% please capitalize first letter and insert punctuation after last keyword.
\keywords{Augmented Reality, Point Registration, Alignment.}
...
aims to decrease end-to-end system latency without significantly sacrificing object placement accuracy using common off-the-shelf AR devices. %\QH{add specific use case here about the close proximity of users as this gives a clear picture of the problem scope.}
...
There are multiple perspectives in which this problem has been approached by the multi-user AR research community.
...
\begin{itemize}
    ...
    \item We have propose a gravity-based user alignment approach that is able to resolve alignment an order of magnitude faster than cloud-based methods in resource-constrained mobile devices. 
...
sequence of spatial data (this may be video or point clouds) that is chosen as a particularly useful frame. Usefulness may 
...
This application consists of several major components: SLAM, object detection, streaming and peer-to-peer communication, and alignment. We measured the latencies incurred by these components and show the results in Table \ref{tab:baseline}. % \MD{MAybe i should just edit the parallel process diagram. I am having trouble coming upwith a good format for the table because i can't actually measure all the time intervals of the diagram individually.}
...
\begin{table}[!htb]
    ...
    \begin{tabular}{|l|r|}
        ...
        \hline
    \end{tabular}
    \caption{Latency breakdown of exploratory system.}
    \label{tab:baseline}
\end{table}
...
respectively.
% We observe that the cloud server host and resolve time is the largest latency bottleneck in the system. \QH{this is still not clear. what's not clear is that you assume people know `the cloud server host and resolve time' is the same as `user alignment' time.}
Given this massive disparity between user alignment latency and other functions, we believe the greatest benefits to user-perceived
...
\begin{figure}[!htb]
    ...
    \caption{Benchmark AR system with users requesting Cloud Anchor hosting from Google servers (cloud).}
...
To achieve a system that provides users with real-time alignment, we propose a hybrid architecture that 
uses peer-to-peer results to supply an almost instant user alignment. The real-time user alignment is then replaced by a higher latency, but
more accurate cloud-based alignment. With this augmentation the overall system adds no new devices, but now the host and resolver devices have 
...
The two distinct approaches that allow this system to perform so much faster are its hybrid architecture and its gravity constrained 
point cloud alignment algorithm that runs extremely fast on resource-constrained user devices.
...
into account to run a user alignment algorithm at periodic intervals to generate new peer-to-peer alignment estimates. By running this algorithm in parallel, 
the same system used for exploratory studies may now facilitate user interaction within approximately 100ms. 
...
\begin{figure}[h]
    ...
    \caption{Hybrid system. The sequence of peer-to-peer streamed point cloud messages are shown in red. The cloud-based alignment is shown in blue.}
...
% \QH{this first sentence reads odd. The flow from the previous paragraph is not good because you started with `... then filters', what about first? } \MD{I beleive this was an artifact from a copy paste.}
...
Experiments were conducted in a variety of well-lit indoor environments. The system begins streaming 
...
This reduction in latency comes with a less spatially accurate alignment between users. Figure \ref{fig:accuracy_histogram} below shows the
...
The average error, seen in Figure \ref{fig:distance_barchart}, in the x, y, and z directions is -0.2133cm, 0.5718cm, and 
-1.0175cm respectively. %\QH{convert these numbers to cm unit} 
...
% Some of this may get implemented by the deadline, but may also not be necessary.
...
the system currently does. This idea was attempted, but with no promising results in the time available.
